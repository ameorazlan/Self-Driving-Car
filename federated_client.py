...
def train_model(model, data_dir, X_train, X_valid, y_train, y_valid, batch_size, nb_epoch, samples_per_epoch, learning_rate):
    checkpoint = ModelCheckpoint('federated_full_1-{epoch:03d}.h5',
                                 monitor='val_loss',
                                 verbose=0,
...
def main():
    ...
    model = build_model()  
    client = FlowerClient(model, args.data_dir, batch_size=40, nb_epoch=1, samples_per_epoch=5530, learning_rate=1e-4, test_size=0.2, keep_prob=0.5)
